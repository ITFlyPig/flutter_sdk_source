...
///
/// 这个类可以让你像这样执行任务：
/// * 正向[forward]或反向[reverse]播放一个动画，或停止[stop]一个动画;
/// * 为动画设置为一个特定的值[value];
/// * 定义一个动画的 [upperBound] 和 [lowerBound] 值;d
/// * 使用物理模拟创建一个甩动[fling]动画效果。
///
/// 默认情况下，[AnimationController]在给定的时间内，线性地产生从0.0到1.0的值。每当运行你
/// 的应用程序的设备准备显示一个新的帧时，AnimationController就会产生一个新的值（通常，这
/// 个速度大约是每秒60个值）。
///
/// ## Ticker providers
///
/// 一个[AnimationController]需要一个[TickerProvider]，它是通过AnimationController的
/// 构造函数的`vsync`参数来配置的。
///
///  TickerProvider]接口描述了一个[Ticker]对象的工厂。[Ticker]知道如何在[SchedulerBinding]
///  上注册自己，并且每一帧都会触发一个回调。AnimationController]类使用一个[Ticker]来逐步完成它所控制的动画。
///
///  如果一个[AnimationController]是由一个[State]创建的，那么这个State可以使用[TickerProviderStateMixin]
///  和[SingleTickerProviderStateMixin]类来实现[TickerProvider]接口。[TickerProviderStateMixin]
///  类总是可以达到这个目的；在类只需要一个[Ticker]的情况下，[SingleTickerProviderStateMixin]的效率略高
///  （例如，如果该类在其整个生命周期中只创建一个[AnimationController]）。
///
///  widget测试框架[WidgetTester]可以在测试的背景下被用作ticker提供者。在其他情况下，你将
///  不得不从更高层次传递一个[TickerProvider]（例如间接地从混合了[TickerProviderStateMixin]
///  的[State]传递），或者创建一个自定义的[TickerProvider]子类。
///
/// ## Life cycle
///
/// 当一个[AnimationController]不再被需要时，应该被处理[dispose]掉。这样可以减少泄露的可
/// 能性。当与[StatefulWidget]一起使用时，[AnimationController]通常在[State.initState]方
/// 法中被创建，然后在[State.dispose]方法中被处理掉。
//
// ##  在[AnimationController]中使用[Future]s
//
//  启动动画的方法会返回一个[TickerFuture]对象，当动画成功完成时，该对象就会完成，并且不会
//  抛出错误；如果动画被取消了，未来就不会完成了。这个对象也有一个[TickerFuture.orCancel]
//  属性，它返回一个当动画成功完成时的future，当动画被中止时，以错误为完成。
//
//  这可以用来编写代码，比如下面的`fadeOutAndUpdateState`方法。
///
/// {@tool snippet}
///
/// 这里是一个有状态的部件`Foo`。它的[State]使用[SingleTickerProviderStateMixin]来实现
/// 必要的[TickerProvider]，在[State.initState]方法中创建其控制器，并在[State.dispose]方
/// 法中State.dispose它。控制器的持续时间是由 "Foo "部件的一个属性配置的；当它发生变化时，
/// [State.didUpdateWidget]方法被用来更新控制器。
///
/// ```dart
...
/// {@tool snippet}
///
/// 下面的方法（对于[State]的子类）使用Dart的await [Future]的异步语法来驱动两个动画控制器。
///
/// ```dart
...
/// {@end-tool}
///
/// 上面代码中的假设是，动画控制器在[State]子类对[State.dispose]方法的覆盖中被处理掉。
/// 由于dispose控制器会取消动画（引发[TickerCanceled]异常），这里的代码可以跳过验证[State.mounted]在每个步骤中是否仍然为真。
/// (同样，这假设控制器在[State.initState]中被创建，并在[State.dispose]中被处置，如上节所述)。
///
/// See also:
///
///  * [Tween], 用于将[AnimationController]转换为其他类型的数值范围的基类。
class AnimationController extends Animation<double>
  ...
  /// 向前开始这个动画（朝向结束）。
  //
  //  返回一个[TickerFuture]，在动画完成时完成。
  //
  //  最近返回的[TickerFuture]，如果有的话，被标记为已被取消，这意味着这个未来永远不会完成，
  //  它的[TickerFuture.orCancel]派生未来完成时出现[TickerCanceled]错误。
  //
  //  在动画过程中，[status]被报告为[AnimationStatus.forward]，当动画结束时达到[upperBound]时，
  //  会切换为[AnimationStatus.completed]。
  ///
